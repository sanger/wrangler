...
def test_valid_barcode_wrangle(app, client, mocked_ss_calls_for_96_rack):
    with app.app_context():
        ss_url = (
            f'http://{current_app.config["SS_HOST"]}'
            f'{current_app.config["SS_TUBE_RACK_ENDPOINT"]}'
        )
        mocked_ss_calls_for_96_rack.add(responses.POST, ss_url, body="{}", status=HTTPStatus.CREATED)
...
def test_size48_wrangle(app, client, mocked_ss_calls_for_48_rack):
    with app.app_context():
        ...
        mocked_ss_calls_for_48_rack.add(responses.POST, ss_url, body="{}", status=HTTPStatus.CREATED)
